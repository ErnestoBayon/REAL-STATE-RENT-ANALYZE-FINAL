...
try:
    from api_data_fetcher import DataEnricher, display_economic_dashboard
    API_AVAILABLE = True
except ImportError:
    API_AVAILABLE = False
...
def main():
    ...
    if API_AVAILABLE:
        try:
            ...
            if census_key and fred_key:
                st.info("🌐 Enriching data with real-time Census & FRED APIs...")
                
                # Initialize API enricher
                enricher = DataEnricher(census_key, fred_key)
                
                # Enrich neighborhood data with real API data
                neighborhoods_df = enricher.enrich_neighborhood_data(neighborhoods_df)
                
                # Get economic indicators
                indicators = enricher.get_economic_indicators()
                
                # Display economic dashboard
                display_economic_dashboard(indicators)
                
                st.success("✅ Data enriched with real Census & FRED data!")
            else:
                st.warning("⚠️ API keys not configured. Using simulated data only.")
        except Exception as e:
            st.warning(f"⚠️ API enrichment failed: {e}. Using simulated data.")
